	...
	<u-modal :show="show" :title="' '" :content="' '" @cancel="onCancel" :showConfirmButton="false">
		<view class="c-improve" v-if="item">
			...
			<view class="c-improve__footer" slot="confirmButton">
				<view class="btn" @click="selectIndex++" v-if="items.length > 1 && selectIndex != items.length - 1">下一个</view>
				<view class="btn" @click="selectIndex--" v-if="items.length > 1 && selectIndex != 0">上一个</view>
				<view class="btn btn2" v-if="items[selectIndex].state != '已整改' && items[selectIndex].state != '已完成'" @click="showChange = true">整改</view>
				<view class="btn btn2" @click="onCancel">确定</view>
			</view>
		</view>
	</u-modal>
</template>
