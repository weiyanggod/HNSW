var calendar = {
	...
	//农历节日 *表示节假日
	lFtv: new Array('0101*春节', '0115 元宵节', '0505 端午节', '0707 七夕情人节', '0715 中元节', '0815 中秋节', '0909 重阳节', '1208 腊八节', '1223 小年', '0100*除夕'),

	//按周计算 月周日
	wFtv: new Array('0520 国际母亲节', '0530 全国助残日', '0630 国际父亲节', '0932 国际和平日', '0940 国际聋人节', '1013 国际减轻自然灾害日', '1011 国际住房日'),
	//输出bug
	...
	calendar: function (y, m) {
		...
		for (var i = 0; i < this.length; i++) {
			...
			this[i] = new calendar.calElement(
				y,
				m + 1,
				i + 1,
				calendar.nStr1[(i + this.firstWeek) % 7],
				lY,
				lM,
				lD++,
				lL,
				calendar.cyclical(lDObj.yearCyl),
				calendar.cyclical(lDObj.monCyl),
				calendar.cyclical(lDObj.dayCyl++)
			)
	...
	drawCld: function (SY, SM) {
		...
		for (i = 0; i < 42; i++) {
			...
			if (sD > -1 && sD < calendar.cld.length) {
				...
				if (calendar.cld[sD].lDay == 1)
					lObj.text =
						(calendar.cld[sD].isLeap ? '闰' : '') +
						calendar.cld[sD].lMonth +
						'月' +
						(calendar.monthDays(calendar.cld[sD].lYear, calendar.cld[sD].lMonth) == 29 ? '小' : '大')
				else lObj.text = calendar.cDay(calendar.cld[sD].lDay)
		...
		calendar.normalDateList = result1
		calendar.lunarDateList = result2
	...
	pushBtm: function (K) {
		...
		switch (K) {
			case 'YU':
				if (this.tY > 1901) this.tY--
				break
			case 'YD':
				if (this.tY < 2049) this.tY++
				break
			case 'MU':
				if (this.tM > 0) {
					this.tM--
	...
	mOvr: function (v) {
		...
		if (calendar.cld[d].solarTerms == '' && calendar.cld[d].solarFestival == '' && calendar.cld[d].lunarFestival == '') {
			festival = ''
		} else {
		...
		datedetail =
			calendar.cld[d].sYear +
			' 年 ' +
			calendar.cld[d].sMonth +
			' 月 ' +
			calendar.cld[d].sDay +
			' 日 星期' +
			calendar.cld[d].week +
			' ' +
			'农历' +
			(calendar.cld[d].isLeap ? '闰 ' : ' ') +
			calendar.cld[d].lMonth +
			' 月 ' +
			calendar.cld[d].lDay +
			' 日 ' +
			calendar.cld[d].cYear +
			'年 ' +
			calendar.cld[d].cMonth +
			'月 ' +
			calendar.cld[d].cDay +
			'日'
