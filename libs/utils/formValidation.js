...
export default {
	...
	*/
	validation: function(formData, rules) {
		for (let item of rules) {
			...
			for (let i = 0, length = rule.length; i < length; i++) {
				...
				switch (ruleItem) {
					case "required":
						isError =this._isNullOrEmpty(formData[key]);
						break;
					case "isMobile":
						isError = !this._isMobile(formData[key]);
						break;
					case "isEmail":
						isError = !this._isEmail(formData[key]);
						break;
					case "isCarNo":
						isError = !this._isCarNo(formData[key]);
						break;
					case "isIdCard":
						isError = !this._isIdCard(formData[key]);
						break;
					case "isAmount":
						isError = !this._isAmount(formData[key]);
						break;
					case "isNum":
						isError = !this._isNum(formData[key]);
						break;
					case "isChinese":
						isError = !this._isChinese(formData[key]);
						break;
					case "isEnglish":
						isError = !this._isEnglish(formData[key]);
						break;
					case "isEnAndNo":
						isError = !this._isEnAndNo(formData[key]);
						break;
					case "isEnOrNo":
						isError = !this._isEnOrNo(formData[key]);
						break;
					case "isSpecial":
						isError =this._isSpecial(formData[key]);
						break;
					case "isEmoji":
						isError =this._isEmoji(formData[key]);
						break;
					case "isDate":
						isError = !this._isDate(formData[key]);
						break;
					case "isUrl":
						isError = !this._isUrl(formData[key]);
						break;
					case "isSame":
						isError = !this._isSame(formData[key], formData[value]);
						break;
					case "range":
						let range = null;
						try {
						...
						isError = !this._isRange(formData[key], range[0], range[1])
						break;
					case "minLength":
						isError = !this._minLength(formData[key], value)
						break;
					case "maxLength":
						isError = !this._maxLength(formData[key], value)
						break;
					default:
						break;
				}
	...
	_isEnOrNo: function(value) {
		...
		let reg = /.*[\u4e00-\u9fa5]+.*$/;
		let result = true;
		if (reg.test(value) ||this._isSpecial(value) ||this._isEmoji(value)) {
			result = false
		}
		return result
	},
